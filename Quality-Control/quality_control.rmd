...
-<sample_data>.txt  If you are starting with a *.vcf*, or your *.fam* does not have the sex /family of the samples already specified please provide an additional file with a header as follows:
...
We prefer plink to calculate sample heterozygosity. Plink uses a *sliding window* approach to identify variants in linkage disequilibrium.
There are many options to modify the behavior or this approach in [plink's documentation](https://www.cog-genomics.org/plink/1.9/ld#indep). 
The LD pruning requires that the *.bim* file has variant IDs in the second column.
If no variants have been assigned, you could do a preliminarystep using [--set-missing-var-ids](https://www.cog-genomics.org/plink/1.9/data#set_missing_var_ids).
...
If there are nor variants in the PAR region, then it will append '.split-x' to ensure compatibility further on. 
Make sure you are using the correct genome alignment for this. e.g. ReDLat files are aligned to hg38. 
Set up `genome_alignment` variable accordingly You can find more information on this step in [plink documentation](https://www.cog-genomics.org/plink/1.9/data#split_x)
...
Detection count thresholds can be modified by changing the following flag: `--check-sex y-only [female max Y obs] [male min Y obs]`
...
`--duplicate` identifies duplicates or MZ twins and generates the file ${prefix}.con, where each line represents two samples with a heterozygote concordance rate > 80%; 
these samples should be carefully examined to determine if they are known biological duplicates (e.g., the same patient sequenced twice), in which case the genome with the best metrics should be retained, or if they are different samples, in which case both genomes should be removed, as it is not possible to determine which genome belongs to which sample.

> If you have known biological replicates I suggest to go over the duplicate list manually and decide which genome to exclude from each pair. 
> If you are not expecting any duplicates, eliminate all of them
...
This step is particularly important if you are working with samples that have known relatedness or are sampling from a small community; however, it is optional and can be toggled on or off in step 2 of the quality control process, 'check_relatedness'
...
In this step, you verify that the disclosed relatedness among individuals matches their genetic relatedness and search for cryptic relatedness between samples; if the *.fam* file lacks a Family Identifier per genome (FID), you need to add that information using Plink's [`--update-ids`](https://www.cog-genomics.org/plink/1.9/data#update_indiv), and if you have parent-offspring samples in your dataset, it is also useful to add this information to the *.fam* file using [`--update-parents`](https://www.cog-genomics.org/plink/1.9/data#update_indiv).
...
If you encounter FATAL ERROR - Please correct problems with pedigree structure, it typically indicates an inconsistency, such as an individual listed as male appearing as the mother of another individual

> HUMAN INPUT NEEDED: YOU must manually analyze the files and create a table of samples with problematic relatedness that need to be excluded (problematic_relatedness.txt); if your data type is GENOME or EXOME, each row should list the individual ID of each sample, whereas for ARRAY data, each row should include `Family ID`, `Individual ID`, ensuring that the provided IDs match those in the .fam file..
...
"If your dataset includes trios, parent-offspring duos, or multigenerational families, you can leverage this information to check for Mendelian inconsistencies using Plink's commands to [identify](https://www.cog-genomics.org/plink/1.9/basic_stats#mendel) and [remove](https://www.cog-genomics.org/plink/1.9/data#set_me_missing) these inconsistencies based on the individuals in your data set."
...
[plugins](https://samtools.github.io/bcftools/howtos/plugin.mendelian.html) for bcftools or
[programs](https://gatk.broadinstitute.org/hc/en-us/articles/360037594831-FindMendelianViolations-Picard-) that do it, however they usually require trios (both parents + offspring). Please refer to the source pages for its usage
...
After refining the dataset, we remove variants and individuals missing more than 5% of the data. 
The inclusion threshold can be adjusted using the `--max-missing` flag for VCF files [0 = all data allowed, 1 = no missing data allowed] or the `--geno` flag for Plink files [1 = all data allowed, 0 = no missing data allowed]."
